...
# backend/ 기준 절대경로 — cwd(프로젝트 루트 vs backend/)에 관계없이 항상 같은 파일을 찾는다.
DOC_PATHS = os.path.join(os.path.dirname(__file__), "sample_docs", "company_doc.pdf")
# CUSTOMIZE: PERSIST_DIR — 트랙별 index 분리 (rag_pipeline.py가 재로드하는 폴더와 동일)
PERSIST_DIR = os.path.join(os.path.dirname(__file__), "chroma_job_docs")
...
# 이미 색인된 폴더가 있으면 재로드만 하고, 없을 때만 PDF를 새로 읽어 색인한다.
# (재시작마다 무조건 from_documents를 호출하면 같은 문서가 계속 중복 누적된다.)
if os.path.exists(PERSIST_DIR):
    job_db = Chroma(
        persist_directory=PERSIST_DIR,
        embedding_function=get_embeddings(),   # 재로드는 embedding_function= 인자
    )
else:
