...
load_dotenv()  # .env의 OPENAI_API_KEY 로드 — 반드시 모델 초기화보다 먼저 실행돼야 해요
...
class InterviewScore(BaseModel):
    ...
    improvements: str = Field(description="답변에서 보완해야 할 점 1~3가지")  # 여기를 채워요 — 모델이 '보완점' 칸에 무엇을 쓰면 되는지 한 문장으로
    next_question: str = Field(description="이어서 물어볼 후속 면접 질문")
prompt = ChatPromptTemplate.from_messages([
    ("system", "당신은 {job_role} 직무 면접관입니다. 후보자 답변을 평가 기준에 따라 채점하세요."),
    # human: 질문과 답변 — question, candidate_answer 변수가 여기에 들어가요
    ("human", "질문: {question}\n후보자 답변: {candidate_answer}\n위 답변을 평가하세요."),
])
model = init_chat_model("openai:gpt-5.4-nano")  # provider prefix 포함 — 항상 "openai:" 붙여요
structured_model = model.with_structured_output(InterviewScore)


chain = prompt | structured_model   # RunnableSequence 조립 — 이 줄에서는 API 호출이 일어나지 않아요

# interview_lcel_step1.py — 3/3: invoke + 샘플 JSON (같은 파일에 이어서 작성해요)
